#!/bin/bash
#SBATCH --job-name=g_masked
#SBATCH --partition=gigabyte_a6000
#SBATCH --gres=gpu:2
#SBATCH --cpus-per-task=16
#SBATCH --time=72:00:00
#SBATCH --output=logs/train_cn_glyph_masked_%j.log

eval "$(conda shell.bash hook)" && conda activate cortext
cd ~/cortext
export PYTHONPATH="$PWD:$PYTHONPATH"

accelerate launch --num_processes 2 training/train_z_image_controlnet_glyph_masked.py \
    ...
    --output_dir /scratch2/shaush/training_output/controlnet_glyph_masked \
    --max_pixels 1048576 \
    --train_batch_size 1 \
