"""Z-Image ControlNet fine-tuning with glyph rendering conditioning."""
...
def parse_args():
    ...
    p.add_argument("--output_dir", type=str, default="z-image-controlnet-glyph")
...
def main():
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("z-image-controlnet-glyph", config=vars(args))
    ...
    if accelerator.is_main_process:
        ...
        for batch in train_dataloader:
            ...
            for i in range(batch["pixel_values"].shape[0]):
                if debug_count >= 10:
                    break
                gt_img = batch["pixel_values"][i]  # (3, H, W), normalized
                gl_img = batch["glyph_values"][i]
                # Denormalize: x * 0.5 + 0.5
                gt_pil = transforms.ToPILImage()(gt_img * 0.5 + 0.5)
                gl_pil = transforms.ToPILImage()(gl_img * 0.5 + 0.5)
                # Side-by-side: GT left, glyph right
                w, h = gt_pil.size
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                with torch.no_grad():
                    ...
                    # Encode glyph canvas through VAE
                ...
                # Loss (full image MSE, like AnyText)
