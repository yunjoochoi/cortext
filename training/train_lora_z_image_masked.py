...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to manifest.jsonl (per-textbox records). Enables loss masking on bbox regions.",
    )
    ...
    if args.manifest_jsonl is None:
        if args.dataset_name is None and args.instance_data_dir is None:
            raise ValueError("Specify `--manifest_jsonl`, `--dataset_name`, or `--instance_data_dir`")
...
def build_text_prompt(caption: str, text: str) -> str:
    """Build training prompt: caption + single text mention."""
    if not caption:
...
def build_bbox_mask(
    ...
    """Pixel bbox → latent-space binary mask [1, latent_H, latent_W]."""
    x, y, w, h = bbox
    scale_x = target_w / orig_w / latent_scale
    scale_y = target_h / orig_h / latent_scale
    lx = max(0, round(x * scale_x))
    ly = max(0, round(y * scale_y))
    lx2 = min(target_w // latent_scale, round((x + w) * scale_x))
    ly2 = min(target_h // latent_scale, round((y + h) * scale_y))
    mask = torch.zeros(1, target_h // latent_scale, target_w // latent_scale)
    if lx2 > lx and ly2 > ly:
        mask[0, ly:ly2, lx:lx2] = 1.0
    return mask
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        # --- Manifest JSONL path: 1 record per text box, each becomes 1 sample ---
        if manifest_records is not None:
            ...
            for rec in manifest_records:
                ...
                instance_images.append(image_cache[img_path].copy())
                self.custom_instance_prompts.append(
                    build_text_prompt(rec.get("caption", ""), rec["text"])
                )
                self.annotation_ids.append(rec.get("annotation_id", ""))

            if repeats > 1:
                instance_images = list(itertools.chain.from_iterable(
                    itertools.repeat(img, repeats) for img in instance_images
                ))
                self.custom_instance_prompts = list(itertools.chain.from_iterable(
                    itertools.repeat(p, repeats) for p in self.custom_instance_prompts
                ))
                self.annotation_ids = list(itertools.chain.from_iterable(
                    itertools.repeat(a, repeats) for a in self.annotation_ids
                ))
                manifest_records = list(itertools.chain.from_iterable(
                    itertools.repeat(r, repeats) for r in manifest_records
                ))
                self._manifest_records = manifest_records
        ...
        if self._manifest_records is not None:
            # manifest path: repeats already applied above
        ...
        for i, image in enumerate(self.instance_images):
            ...
            # Build bbox mask for manifest records
            if self._manifest_records is not None:
                rec = self._manifest_records[i]
                mask = build_bbox_mask(
                    rec["bbox"], rec["width"], rec["height"],
    ...
    def __getitem__(self, index):
        ...
        if self.masks:
            example["mask"] = self.masks[index % self.num_instance_images]
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    if "mask" in examples[0]:
        ...
        if with_prior_preservation:
            masks += [torch.ones_like(masks[0])] * len(examples)
...
def main(args):
    ...
    # Load manifest records if provided — disable all augmentation for bbox mask consistency
    manifest_records = None
    if args.manifest_jsonl:
        ...
        manifest_records = []
        with open(args.manifest_jsonl, encoding="utf-8") as _f:
            for _line in _f:
                _line = _line.strip()
                if _line:
                    manifest_records.append(_json.loads(_line))
        logger.info(f"Loaded manifest: {len(manifest_records)} text boxes from {args.manifest_jsonl}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                # Retrieve cached masks if available
                if masks_cache:
                    batch["masks"] = masks_cache[step]
                ...
                if args.with_prior_preservation:
                    ...
                    if "masks" in batch:
                        batch["masks"], _ = torch.chunk(batch["masks"], 2, dim=0)
                ...
                if "masks" in batch:
                    mask = batch["masks"].to(device=mse.device, dtype=mse.dtype)
                    loss = (mse * mask).sum(dim=[1, 2, 3]) / mask.sum(dim=[1, 2, 3]).clamp(min=1.0)
                    loss = loss.mean()
                else:
                    loss = mse.reshape(target.shape[0], -1).mean(1).mean()
