"""Z-Image ControlNet glyph fine-tuning with 3-phase curriculum (easy -> medium -> hard).

Approach:
  - ControlNet conditions on glyph-rendered latents via control_layers "0,2,4,6,8".
  - Curriculum tiers are computed at load time from per-annotation difficulty scores
    (sum of annotation difficulties per image, split into terciles).
  - Each phase trains on its tier's data with a fresh optimizer + LR scheduler.
  - ControlNet weights carry over across phases (no reload from disk).

accelerate launch --num_processes 2 training/train_z_image_controlnet_glyph_curriculum.py \
    --pretrained_model_name_or_path /path/to/Z-Image \
...
def assign_tiers(records: list[dict]) -> dict[str, list[dict]]:
    """Compute image-level difficulty (sum of annotation difficulties) and split into terciles."""
    scored = []
    for rec in records:
...
def parse_args():
    ...
    p.add_argument("--output_dir", type=str, default="z-image-controlnet-glyph-curriculum")
    ...
    p.add_argument("--control_layers", type=str, default="0,2,4,6,8")
...
def train_phase(
    ...
    # Fresh optimizer + LR scheduler per phase
...
def main():
    ...
    # ---- Prepare ControlNet (shared across phases) ----
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("z-image-controlnet-glyph-curriculum", config=vars(args))
